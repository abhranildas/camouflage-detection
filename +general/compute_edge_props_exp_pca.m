...
% dumb down the efficient bins to only use equispaced bins at each scale
n_bins=size(grad_pca_m_bins,2)-1;
for i_scale=1:size(grad_pca_m_bins,1)
    grad_pca_m_bins(i_scale,:)=[-inf linspace(grad_pca_m_bins(i_scale,2),grad_pca_m_bins(i_scale,end-1),n_bins-1) inf];
    grad_pca_o_bins(i_scale,:)=[-inf linspace(grad_pca_o_bins(i_scale,2),grad_pca_o_bins(i_scale,end-1),n_bins-1) inf];
    grad_pca_p_bins(i_scale,:)=[-inf linspace(grad_pca_p_bins(i_scale,2),grad_pca_p_bins(i_scale,end-1),n_bins-1) inf];
end
...
for i = 1:length(exp_paths)
    ...
    save(['exp_files/' filepath '/edge_props_pca.mat'],'edge_props');
    toc
end
