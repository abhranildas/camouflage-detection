...
% if exist('global_data/edge_powers_pn.mat','file')==2
%     load('global_data/edge_powers_pn.mat')
...
parfor seed=sample_start:sample_start+n_samples-1
    ...
    % target
    [stim,mask,mask_edge,mask_normal]=lib.stimulus('target_radius',64,'ml_b',.5,'cont_b',.15);
    edge_t=lib.edge_vector(stim,'mask',mask,'mask_edge',mask_edge,'mask_normal',mask_normal);
    %     edges_t(seed,:)=edge_t;
    ...
    % blank
    stim=lib.stimulus('ml_b',.5,'cont_b',.15);
    edge_b=lib.edge_vector(stim,'mask',mask,'mask_edge',mask_edge,'mask_normal',mask_normal);
    %     edges_b(seed,:)=edge_b;
...
results=classify_normals(edge_powers(:,1),edge_powers(:,2),'input_type','samp');
...
% save('global_data/edge_measures_8.mat','edges','edge_powers','edge_ps','edge_lpr','-v7.3')
% save('global_data/edge_powers_pn.mat','edge_powers')