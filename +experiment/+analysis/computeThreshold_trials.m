function [mu, threshold, b, c, mu_sd, threshold_sd, b_sd, c_sd, sessions_completed, target_means, pcBoot_sds, x_pmf, y_pmf] = computeThreshold_trials(expTypeStr, subjectStr, bPlot, bBootstrap)
    ...
    load(['vislab-common/data/edge_powers_' expTypeStr '.mat'],'edge_powers');
    ...
    if bPlot
        ...
        fplot(@(x) experiment.analysis.psychometricFun_trials(x,mu,a,b,c,y_blanks),[mu 1],'k');
    ...
    if(bBootstrap)
        n_boot=500;
        pc_boot=nan(n_boot,nLevels); mu_boot=nan(1,n_boot); a_boot=nan(1,n_boot); b_boot=nan(1,n_boot); c_boot=nan(1,n_boot);
        for iBoot= 1:n_boot
            ...
            [mu_boot(iBoot),a_boot(iBoot),b_boot(iBoot),c_boot(iBoot)]=experiment.analysis.fitPsychometric(mu,a,b,c,target_means_boot, num_blanks_boot,num_targets_boot, num_hits_boot, num_cr_boot);
        end
