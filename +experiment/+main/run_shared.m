function SessionData = run_shared(subjectStr, expTypeStr, condition, sessionNumber, levelNumber)
% RUN_SHARED  Launch the camouflage detection experiment via the shared harness.
%   run_shared(subjectStr, expTypeStr [, condition, sessionNumber, levelNumber])
%
%   Behaviour-equivalent replacement for runCamouflageExperiment.m that delegates
%   the session->level->trial loop, screen setup, and teardown to the shared
%   vision-commons harness (psychexp.run_experiment), wiring this repo's existing
%   interval functions (fixationInterval / stimulusInterval / responseInterval /
%   giveFeedback / displayLevelStart) and its EyeLink layer as hooks. The original
%   runCamouflageExperiment.m is kept until this has been validated on a
%   Psychtoolbox machine (see CLEANUP.md); this shares the loop across repos.
%
