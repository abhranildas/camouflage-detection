function [cube_1f,wn]=create_pink_noise_cube(bg_size,exponent)
    % creates a cube of pink noise of given size
    ...
    fil_1f = ones(bg_size,bg_size,bg_size);
    for i=1:bg_size
        for j=1:bg_size
            for k=1:bg_size
                z=norm([i-bg_size/2-1,j-bg_size/2-1,k-bg_size/2-1])^(3/2);
                if z  % leave fft origin at 1
                    fil_1f(i,j,k) = z^(-exponent);
                end
            end
        end
    end
    % white noise image:
    wn = normrnd(0,1,[bg_size bg_size bg_size]);
    ...
    cube_1f = ifftn(ifftshift(wnf_fil));